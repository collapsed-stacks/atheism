...
> How do you concisely point out the
> logical fallacy of theists applying
> Occam's Razor to creation or other
> events, where they claim that God is
> the simplest explanation (just one
> being), so likely to be true?
...
* The early Ptolemaic picture of the solar system put the earth in the center with all the planets orbiting around in perfect circles.
* Unfortunately, calculations of planet's actual motion did not match up with predictions from the perfect circle model, a model was created in which planets orbit in small perfect circles and the focal point of the planetary orbit traced perfect circles around the earth. The smaller circle was called the epicycle, and the end result was a much more accurate prediction of planetary motion.
* However, the model with epicycles is only accurate for a very short period of time, after which the margin of inaccuracy grows to the point that epicycles are no longer adequate to explain planetary motion. So, a model was constructed which was epicycles on top of epicycles, which helped, but only temporarily. As inaccuracies are discovered, astronomers attempt to model epicycles on epicyles (up to 60 or 80 circles in circles).
* Later observations and predictions by Copernicus abandoned the earth-centered model in favor of a sun-centered model, where planets orbit around the sun in epicycles (on the order about about 20 or 30).
* Tycho Brahe refined the heliocentric model by providing evidence against "immutable celestial spheres", he described the motion of comets and supernova which have motions outside and separate from the solar system.
* Kepler went a step further, and while attempting to simplify the epicycles on epicycles theory, he discovered planetary motion could be modeled with ellipses where the sun formed one focal point.
...
The problem with claiming that the existence of a deity that created everything is that something **extra** is introduced _without_ simplifying the explanation _at all_—there's still the matter of _why_ any god would do such a thing and as for the _mechanisms_ that were used, and why they were selected, we're back to square one except with a larger burden of proof.
...
<p><a href="http://lesswrong.com/lw/jp/occams_razor/" rel="nofollow">This argument by Eliezer Yudkowsky</a> is not particularly concise, but is very good, in my opinion. Here is an excerpt:</p>
...
<blockquote>
  ...
  <p>One observes that the length of an English sentence is not a good way to measure "complexity".  And "fitting" the facts by merely failing to prohibit them is insufficient.</p>
...
Even modern science is technically more complex than this since it posits the existence of many more types of particles than just one (consider the Higgs boson, quarks, etc.). Technically then, this fails the simplicity by entity comparison. 
This also is *more simple* than most justifications of theism via Ockham's Razor. Then, if they wish to appeal to simplicity and simplicity alone, they must accept this new theism.
...
- an eternal universe without creation
- a universe that self-generated out of nothing ("nothing" being simpler than any entity)
